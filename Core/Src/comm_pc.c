...
/*
 * 上位机原始接收桥接层�? *
 * 本文件负责两件事�? * - 使用 DMA + UART IDLE 接收 PC / Jetson 的原始字节流，并打印 hex/ascii 方便调试�? * - 将同一份字节流送入 comm_protocol，由协议层负责找帧、CRC 校验、命令入队和 ACK�? *
 * 注意�? * - 这里不直接执行底盘、舵机、传感器业务命令�? * - 如果串口助手只发 hello\r\n，会看到原始回显日志，但不会触发正式协议命令�? */
...
static void HostRx_AppendReceivedData(HostRx_Source_t source, const uint8_t *data, uint16_t length)
{
  ...
  /*
   * 同一份原始接收数据同时进入协议层�?   * 协议层能够处理拆包、粘包和连续多帧；这里不需要按帧切分�?   */
...
{
  /* 先执行正式协议命令，再输出原始收包日志，方便观察命令�?ACK�?*/
  HostProtocol_Poll();
